...
public class AES {
	 ...
	 */
	public static final String newline = System.getProperty("line.separator"); // The
																				// newline
																				// for
																				// whatever
																				// system
	...
	static int keyFileIndex = 1; // Index where the keyFile argument should be.
									// Used to determines the index of other
									// arguments.
	...
	/**
	 * Main method with which we run the AES algorithm. Usage: java AES e|d
	 * [-length] [-mode] keyFile inputFile
	 * 
	 * @param args
...
/*		try {
			key = String.format("%040x", new BigInteger(1, key.getBytes("UTF-8")));
		...
		if (mode == Mode.CBC) // Parse Initialization Vector
		{
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					initvector[j][i] = Integer.parseInt(
							key.substring((8 * i) + (2 * j), (8 * i)
									+ (2 * j + 2)), 16);
				}
			}
		}
		...
		for (int i = 0; i < state.length; i++) // Parses line into a matrix
		{
			for (int j = 0; j < state[0].length; j++) {
				state[j][i] = Integer.parseInt(line.substring(
		...
		try {
			...
			return new String(bytes, "UTF-8").replaceAll("[^\\x20-\\x7e]",
					"");
...
/*		try {
			key = String.format("%040x", new BigInteger(1, key.getBytes("UTF-8")));
			System.out.println("hexed key "+key);
		} catch (UnsupportedEncodingException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}*/
		...
		try {
			line = String.format("%040x", new BigInteger(1, line.getBytes("UTF-8")));
		...
		if (line.length() < 32) {
			...
			System.out.println("after padding or formatting " + line);
		}
		state = new int[4][4];
		...
		addRoundKey(state, subKey(keymatrix, 0)); // Starts the addRoundKey with
													// the first part of Key
													// Expansion
		...
		//System.out.println(temp);// If all systems could just use the same
									// newline, I'd be set.
		return temp;
	...
	private static void deepCopy2DArray(int[][] destination, int[][] source) {
		assert destination.length == source.length
				&& destination[0].length == source[0].length;
	 ...
	 * 
	 * @param km
	 *            key formed from AES.keySchedule()
	 * @param begin
	 *            index of where to fetch the subkey
	 ...
	 * 
	 * @param arr
	 *            Array whose value will be replaced
	...
	/**
	 * Performs a left shift on each row of the matrix. Left shifts the nth row
	 * n-1 times.
	 * 
	 * @param arr
	 *            the reference of the array to perform the rotations.
	 */
	 ...
	 * 
	 * @param arr
	 *            the array to compute right rotations.
	 */
	 ...
	 * 
	 * @param arr
	 *            the array with we calculate against the galois field matrix.
	 */
	...
	/**
	 * Helper method of mixColumns in which compute the mixColumn formula on
	 * each element.
	 * 
	 * @param arr
	 *            passed in current matrix
	 * @param g
	 *            the galois field
	 * @param i
	 *            the row position
	...
	private static int invMcHelper(int[][] arr, int[][] igalois, int i, int j) // Helper
																				// method
																				// for
																				// invMixColumns
	{
	...
	/**
	 * The keyScheduling algorithm to expand a short key into a number of
	 * separate round keys.
	 * 
	 * @param key
	 *            the key in which key expansion will be computed upon.
	 * @return the fully computed expanded key for the AES
	...
	public static int[][] keySchedule(String key) {
		...
		int colsize = binkeysize + 48 - (32 * ((binkeysize / 64) - 2)); // size
																		// of
																		// key
																		// scheduling
																		// will
																		...
																		// the
																		// key.
		int[][] keyMatrix = new int[4][colsize / 4]; // creates the matrix for
														// key scheduling
		int rconpointer = 1;
		int[] t = new int[4];
		...
		for (int i = 0; i < keycounter; i++) // the first 1 (128-bit key) or 2
												// (256-bit key) set(s) of 4x4
												// matrices are filled with the
												// key.
		{
			for (int j = 0; j < 4; j++) {
				keyMatrix[j][i] = Integer
						.parseInt(
								key.substring((8 * i) + (2 * j), (8 * i)
										+ (2 * j + 2)), 16);
			}
		...
		while (keypoint < (colsize / 4)) {
			...
			if (temp == 0) {
				...
				for (k = 0; k < 4; k++) {
					keyMatrix[k][keypoint] = t[k]
							^ keyMatrix[k][keypoint - keycounter];
				}
				keypoint++;
			} else if (temp == 4) {
				for (k = 0; k < 4; k++) {
					int hex = keyMatrix[k][keypoint - 1];
					keyMatrix[k][keypoint] = sbox[hex / 16][hex % 16]
							^ keyMatrix[k][keypoint - keycounter];
				}
				keypoint++;
			} else {
				...
				while (keypoint < ktemp) {
					for (k = 0; k < 4; k++) {
						keyMatrix[k][keypoint] = keyMatrix[k][keypoint - 1]
								^ keyMatrix[k][keypoint - keycounter];
					}
					keypoint++;
				}
	 ...
	 * round, a chunk of the key scheduled is pulled; each subkey is the same
	 * size as the state. Each element in the byte matrix is XOR'd with each
	 * element in the chunk of the expanded key.
	 * 
	 * @param state
	 *            reference of the matrix in which addRoundKey will be computed
	 *            upon.
	 * @param keymatrix
	 *            chunk of the expanded key
	 */
	...
	public static String MatrixToString(int[][] m) // takes in a matrix and
													// converts it into a line
